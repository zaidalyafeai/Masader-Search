...
class Str(BaseType):
    ...
    def compare(self, attr1, attr2, exact_match = False):
        if exact_match:
            return int(attr1.lower() == attr2.lower())
        if len(attr1) == len(attr2) == 0:
            return 1
        else:
            return 1 - levenshtein_distance(attr1.lower(), attr2.lower()) / max(len(attr1), len(attr2)) # TODO: revise
...
class LongStr(Str):
    def compare(self, attr1, attr2, exact_match = False):
        if exact_match:
            return int(attr1.lower() == attr2.lower())
        # use rouge score
        results = scorer.score(attr1.lower(), attr2.lower())
